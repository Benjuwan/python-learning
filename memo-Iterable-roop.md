...
- リストでの要素の取り扱い
  ...
  # extend メソッド
  リスト.extend(イテラブル)
  # JavaScript でいうconcatメソッドによる結合と近いニュアンス
  # リスト.concat(イテラブル)
...
- ※タプルはイミュータブルなので要素の変更や追加、削除といった編集操作は受け付けない
```py
# 9までの tuple[int, ...] を生成
tuple(range(10))
...
- 集合での要素の取り扱い
  ...
  - **複数要素**をまとめて編集（追加・削除）する集合に特有の演算
    - 複数の集合から**新しい集合**を作成<br>
    | 使い方             | 結果                                                      |
    |--------------------|-----------------------------------------------------------|
    | `集合A | 集合B`   | 集合Aまたは集合Bに含まれる要素の集合（和集合） ※`|`はパイプラインと置換 |
    ...
    - 累算代入文：集合によって左辺（集合A）の**既存内容を編集**<br>
    | 使い方             | 結果                                                      |
    |--------------------|-----------------------------------------------------------|
    | `集合A |= 集合B`  | 集合Aに集合Bの要素を追加（和集合の操作）                   |
...
- 辞書での要素の取り扱い
  ...
  - `辞書[キー] = 要素`<br>
  辞書に要素を追加。指定したキーが当該辞書に含まれていない場合は、**キーと値の組が新たに追加**される。含まれている場合は、そのキーに対応する値が書き換えられる。
