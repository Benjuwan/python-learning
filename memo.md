...
- 参照：[`Python`での配列操作について（シャローコピーの必要性有無）](##Pythonでの配列操作について（シャローコピーの必要性有無）)
...
より念入りに属性への操作を制限したい場合、**属性名の先頭に2個のアンダースコア`__`を付ける**ことで、自動的に`_クラス名__属性名`というような属性名に変換される。<br>
...
>   - **プロテクティッド（`protected`）** <br>当該クラス及び子孫クラス専用。当該クラスと、その子クラスからしか呼び出せない（使用できない）<br>家族共用のリビングのように、家族（子クラス含む）なら使えるが、外部の人は入れない。
